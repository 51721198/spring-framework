...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	protected void addSingleton(String beanName, Object singletonObject) {
		synchronized (this.singletonObjects) {
			...
			this.singletonFactories.remove(beanName);    //bean成功创建后需要删除factory
			this.earlySingletonObjects.remove(beanName);
			this.registeredSingletons.add(beanName);    //registeredSingletons是一个set
		}
	}
			...
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {  //允许早引用意味着允许循环引用
					...
					if (singletonFactory != null) {  //beanName  -> Objfactory 的map
						singletonObject = singletonFactory.getObject();   //factoryBean要通过getObj才能获取到bean,这里如果为null了,这里会进入回调方法
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				if (this.singletonsCurrentlyInDestruction) {  //如果bean正在销毁中那么抛出异常
				...
				beforeSingletonCreation(beanName);  //如果bean正在创建中那么抛出异常
				...
				try {
					...
					afterSingletonCreation(beanName);//如果bean正在创建中那么抛出异常
				}
				if (newSingleton) {
					addSingleton(beanName, singletonObject);
				}
	...
	public boolean isSingletonCurrentlyInCreation(String beanName) {
		return this.singletonsCurrentlyInCreation.contains(beanName); //卧槽,这么简单粗暴
	}
